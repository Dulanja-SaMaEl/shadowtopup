<x-admin-layout>
    <div class="mb-10">
        ...
        <h2 class="text-3xl font-black text-white uppercase tracking-tight">Edit Package: <span class="text-purple-400">{{ $package->package_name }}</span></h2>
        <p class="text-gray-400 text-sm mt-1">Update diamond amounts, shell costs, and active visibility status.</p>
    </div>
        ...
        <form action="{{ route('admin.packages.update', $package) }}" method="POST">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="package_name" value="{{ old('package_name', $package->package_name) }}" class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner" required>
                ...
                <div>
                    ...
                    <select name="package_type" class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:ring-purple-500 focus:border-purple-500 transition shadow-inner" required>
                        <option value="diamond" {{ old('package_type', $package->package_type) == 'diamond' ? 'selected' : '' }} class="bg-gray-900 text-white">Diamond Pack</option>
                        <option value="membership" {{ old('package_type', $package->package_type) == 'membership' ? 'selected' : '' }} class="bg-gray-900 text-white">Membership Pack</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="diamond_amount" value="{{ old('diamond_amount', $package->diamond_amount) }}" class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white font-mono placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                ...
                <div>
                    ...
                    <input type="number" name="shell_cost" value="{{ old('shell_cost', $package->shell_cost) }}" class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white font-mono placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner" required>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $package->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded bg-white/5 border-white/10 text-purple-600 focus:ring-purple-500">
