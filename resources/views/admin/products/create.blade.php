<x-admin-layout>
    <div class="mb-10">
        ...
        <h2 class="text-3xl font-black text-white uppercase tracking-tight">Add New Product</h2>
        <p class="text-gray-400 text-sm mt-1">Create a game top-up item and configure pricing tiers.</p>
    </div>
    ...
    <div class="max-w-4xl glass-card rounded-2xl border border-white/10 shadow-2xl p-8">
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="game_id" id="game_id" required class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                            ...
                            @foreach($games as $game)
                                <option value="{{ $game->id }}" {{ old('game_id') == $game->id ? 'selected' : '' }} class="bg-gray-900 text-white">{{ $game->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. 100 Diamonds + 10 Bonus" required class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                        @error('name') <p class="mt-1.5 text-xs text-rose-400 font-bold">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="normal_price" class="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Normal Tier Price (LKR / USD)</label>
                        <div class="relative">
                            <span class="absolute left-4 top-3 text-gray-400 font-mono font-bold">$</span>
                            <input type="number" step="0.01" name="normal_price" id="normal_price" value="{{ old('normal_price') }}" required class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 pl-9 text-sm text-white font-mono placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="silver_price" id="silver_price" value="{{ old('silver_price', 0) }}" required class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 pl-9 text-sm text-white font-mono placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="gold_price" id="gold_price" value="{{ old('gold_price', 0) }}" required class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 pl-9 text-sm text-white font-mono placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">
                        </div>
                ...
                <div class="space-y-6">
                    <div>
                        <label class="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Product Thumbnail Image (Optional)</label>
                        <div class="relative group h-48 w-full">
                            ...
                            <label for="image" class="w-full h-full border-2 border-dashed border-white/10 hover:border-purple-500/50 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition bg-white/5 overflow-hidden">
                                <img id="product-preview" class="hidden w-full h-full object-contain p-2">
                                <div id="product-placeholder" class="text-center p-4">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10">
                        <input type="checkbox" name="is_published" id="is_published" value="1" checked class="w-4 h-4 rounded bg-white/5 border-white/10 text-purple-600 focus:ring-purple-500">
                        <label for="is_published" class="text-xs font-bold text-gray-200 uppercase tracking-widest cursor-pointer">Publish Immediately</label>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Additional details or delivery notes..." class="w-full bg-white/5 border-white/10 rounded-xl py-3 px-4 text-sm text-white placeholder-gray-500 focus:ring-purple-500 focus:border-purple-500 transition shadow-inner">{{ old('description') }}</textarea>
